package MatFel::Controller::Servidores;
...
sub buscar : Local :Args(1) {
	...
	my $id_user=$c->user->get('id');
	$c->stash->{servidores} = [$c->model('DB::Servidor')->search({
	-and => [
      		-or => [
	...
	#$c->stash->{servidores} = [$c->model('DB::Servidor')->search({[{'user' => $id_user}] ,[{'nombre' => {'like' , '%'.$busqueda.'%'}}, {'ipv4' =>{'like' , '%'.$busqueda.'%'}}]})];
#	({'buscar_user.id' => $c->user->id,'evento.timestamp' => {'>=', $c->user->last_login->timestamp}}
...
=head2 reporte
    ...
    habilitar/deshabilitar el reporte del servidor
    
=cut
    
sub reporte :Chained('object') :PathPart('reporte') :Args(1) {
        my ($self, $c, $reporte) = @_;
        #FIXME hay q arreglar los permisos
    if ($c->user->get('id') eq $c->stash->{object}->user()) {
        # Use the book object saved by 'object' and delete it along
        # with related 'book_author' entries
       my $nombre=$c->stash->{object}->nombre_dir;
        $c->stash->{object}->update({ reporte => $reporte,});
        ...
        # Set a status message to be displayed at the top of the view
        $c->stash->{status_msg} = "Servidor ".$nombre;
      if ($reporte) {$c->stash->{status_msg} .=" reporte.";} 
          else {$c->stash->{status_msg} .="reporte .";} 
    }
    else
    {
    $c->stash->{status_msg} = "Usted no tiene permisos sobre ese Servidor para poder modificar su habilitación o no.";
