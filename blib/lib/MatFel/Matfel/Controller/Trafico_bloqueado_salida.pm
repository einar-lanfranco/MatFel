package MatFel::Controller::Trafico_bloqueado_salida;
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched MatFel::Controller::Trafico_bloqueado_salida.');
}
...
sub base :Chained('/') :PathPart('trafico') :CaptureArgs(0) {
        ...
        $c->stash->{resultset} = $c->model('DB::Trafico_salida_bloqueado');
...
sub borrar :Chained('object') :PathPart('borrar_saliente_bloqueado') :Args(0) {
        my ($self, $c) = @_;
        ...
        $c->stash->{status_msg} = "Trafico de Salida Bloqueado Borrado.";
