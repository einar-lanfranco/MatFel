package MatFel::Controller::Trafico_salida;
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched MatFel::Controller::Trafico_salida.');
}
...
sub base :Chained('/') :PathPart('trafico') :CaptureArgs(0) {
        ...
        $c->stash->{resultset} = $c->model('DB::Trafico_salida');
...
Usar create para agregar nuevo trafico saliente al servidor
    
=cut
    
sub create :Chained('server') :PathPart('create_saliente') :Args(0) :FormConfig('trafico/saliente_create.conf') {
        my ($self, $c) = @_;
        # Get the form that the :FormConfig attribute saved in the stash
        my $form = $c->stash->{form};
        if ($c->user->get('id') eq  $c->stash->{object}->user()) {
        if ($form->submitted_and_valid) {
            # Create a new trafico_saliente
            my $trafico_saliente = $c->model('DB::Trafico_salida')->new_result({});
            # Save the form data for the book
            $trafico_saliente->id_servidor($c->stash->{object}->id());
            $form->model->update($trafico_saliente);
    ...
    }else
  {
       ...
       $c->stash->{template} = 'trafico/saliente_create.tt2';
   
}


sub edit :Chained('object') :PathPart('edit_saliente') :CapturArgs(0) 
            :FormConfig('trafico/saliente_create.conf') {
        my ($self, $c) = @_;
...
sub borrar :Chained('object') :PathPart('borrar_saliente') :Args(0) {
        my ($self, $c) = @_;
        ...
        $c->stash->{status_msg} = "Trafico de Salida Eliminado Borrado.";
...
sub habilitar_salida :Chained('object') :PathPart('habilitar_salida') :Args(1) {
    ...
    if ($c->user->get('id') eq $c->stash->{object}->id_servidor->user()) {
       ...
       my $nombre=$c->stash->{object}->puerto."/".$c->stash->{object}->protocolo->nombre;
        ...
        # Set a status message to be displayed at the top of the view
        $c->stash->{status_msg} = "Tráfico saliente a ".$nombre;
      if ($habilitado) {$c->stash->{status_msg} .=" habilitado.";} 
          else {$c->stash->{status_msg} .=" deshabilitado.";} 
    }
    else
    {
    $c->stash->{status_msg} = "Usted no tiene permisos sobre ese Servidor para poder modificar su habilitación o no de tráfico saliente.";
    }
        # Forward to the list action/method in this controller
        $c->response->redirect($c->uri_for($c->controller('trafico')->action_for('listar_trafico'),[$c->stash->{object}->id_servidor->id()])); 
    }
