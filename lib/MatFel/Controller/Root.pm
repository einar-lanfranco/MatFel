...
sub index :Path :Args(0) {
    ...
    $today[1]++;
    my $days;
    my @alertas_entre_logins;
    if (!defined($c->user->last_login)) 
    { #first time login
		$days=1;
    }
    else {
    ...
    @alertas_entre_logins=$c->model('DB::Alerta')->search({'buscar_user.id' => $c->user->id,'evento.timestamp' => {'>=', $ultimos}}, {join => ['evento', {'id_servidor'=>'buscar_user'}], select => [{ count => '*', -as => 'cuenta'},'id_servidor.nombre','id_servidor.id'], as => [qw/ cuenta server_nombre id_servidor /], group_by => 'id_servidor.id', order_by => {-desc => 'cuenta'}});
    }
