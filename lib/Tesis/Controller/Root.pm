...
Tesis::Controller::Root - Root Controller for Tesis
...
if ($c->user->last_login){
my @alertas_entre_logins=$c->model('DB::Alerta')->search({'buscar_user.id' => $c->user->id,'evento.timestamp' => {'>=', $c->user->last_login->timestamp}}, {join => ['evento', {'id_servidor'=>'buscar_user'}], select => [{ count => '*', -as => 'cuenta'},'id_servidor.nombre','id_servidor.id'], as => [qw/ cuenta server_nombre id_servidor /], group_by => 'id_servidor.id', order_by => {-desc => 'cuenta'}});
...
if ($c->request->address){
   $c->stash->{geo_login} = Tesis::Controller::Utilidades::get_geo_ip($c->request->address,$c);
}
