...
Tesis::Controller::OFC - Catalyst Controller
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched Tesis::Controller::OFC in OFC.');
}
...
sub historico_vulnerabilidades :Chained('base') :PathPart('historico_vulnerabilidades') :Args(2) {
    ...
    my $fechas_vuln =$c->model('DB::OV_scanresults')->search({id_scan => $id}, {'columns' => ['timestamp'],distinct => 1});
    ...
    while( my $dia = $fechas_vuln->next ) {
	    ...
	    my $escaneo_dia=$escaneo_total->search({timestamp =>$dia->timestamp});
	    my @cantidades=(0,0,0);
	    my $max =0;
