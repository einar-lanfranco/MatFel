package Tesis::Controller::Escaneos;
...
Tesis::Controller::Escaneos - Catalyst Controller
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched Tesis::Controller::Escaneos in Escaneos.');
}
...
sub escaneos_historicos :Chained('object') :PathPart('escaneos_historicos') :Args(1)  {
	...
	my $ov_scan=$c->stash->{object};
	my $fechas_vuln =$c->model('DB::OV_scanresults')->search({id_scan => $ov_scan->id}, {'columns' => ['timestamp'], distinct => 1});
