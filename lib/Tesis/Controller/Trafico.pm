package Tesis::Controller::Trafico;
...
sub index :Path :Args(0) {
    ...
    $c->response->body('Matched Tesis::Controller::Trafico.');
}
  ...
  my ($self, $c, $ip ,$port, $proto) = @_;
    ...
    if ($c->user->get('id') eq $c->stash->{object}->user()) {
        ...
        my $tbe = $c->model('DB::Trafico_entrada_bloqueado')->find_or_create({
          ...
          $c->stash->{status_msg} = "El tráfico al puerto ".$port." (".$proto.") desde la ip ".Tesis::Controller::Utilidades::dec2dot($ip)." ha sido bloqueado.";
        }
  else
    {
          $c->stash->{status_msg} = "Usted no tiene permisos sobre ese Servidor para poder ver mas informacion.";
  ...
  my ($self, $c, $ip ,$port, $proto) = @_;
    ...
    if ($c->user->get('id') eq $c->stash->{object}->user()) {
        ...
        my $tbs = $c->model('DB::Trafico_salida_bloqueado')->find_or_create({
          ...
          $c->stash->{status_msg} = "El tráfico del puerto ".$port." (".$proto.") a la ip ".Tesis::Controller::Utilidades::dec2dot($ip)." ha sido bloqueado.";
        }
  else
    {
          $c->stash->{status_msg} = "Usted no tiene permisos sobre ese Servidor para poder ver mas informacion.";
  ...
  my ($self, $c, $ip ,$port, $proto) = @_;
    ...
    if ($c->user->get('id') eq $c->stash->{object}->user()) {
        ...
        my $tbs = $c->model('DB::Trafico_salida_bloqueado')->find_or_create({
          ...
          $c->stash->{status_msg} = "El tráfico al puerto ".$port."(".$proto.") a y desde la ip ".Tesis::Controller::Utilidades::dec2dot($ip)." ha sido bloqueado.";
        }
  else
    {
          $c->stash->{status_msg} = "Usted no tiene permisos sobre ese Servidor para poder ver mas informacion.";
