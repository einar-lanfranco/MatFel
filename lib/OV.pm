...
sub run_scan_catalyzed {
	...
	else
	{
	    ...
	    if ($resultado_escaneo){
	      $c->model('DB')->schema->populate('OV_scanresults', [[qw/id_scan IP Port OID Type Description risk CVE/],@$resultado_escaneo]);
...
sub run_scan {
	...
	else
	{
	    ...
	    if ($resultado_escaneo){
	      my $schema=getSchema();
	      $schema->populate('OV_scanresults', [[qw/id_scan IP Port OID Type Description risk CVE/],@$resultado_escaneo]);
	    }
...
push (@resultado_escaneo,[$id,$values[2],$values[3],$values[4],$values[5],$values[6],$idRisk,$cve])
